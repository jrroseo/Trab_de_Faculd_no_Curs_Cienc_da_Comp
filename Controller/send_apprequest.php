<?php
	...
	$patient_query = mysqli_query($conn,"SELECT patient_id, patient_username, patient_lname FROM patient WHERE patient_username='$username'");
	...
	$query = "INSERT INTO appointment (app_patient_id,app_number, app_patientname, app_doctorname, app_time, app_date, app_hospital, app_status, app_patientusername, app_doctorusername)
			VALUES ('{$patient_array[0]}','{$app_no[0]}', '{$patient_name}', '{$doctor_name}', '{$_POST['app_time']}', '{$_POST['app_date']}', '{$doctor_array[2]}', 'Aprovado', '{$username}', '{$doctor_id}')";
	$result = mysqli_query($conn,$query);
    }
	mysqli_close($conn);

	//send_notification($username, $doctor_id, 1);
	header('Location: ../View/paciente/agendamentos_patient.php');
?> 